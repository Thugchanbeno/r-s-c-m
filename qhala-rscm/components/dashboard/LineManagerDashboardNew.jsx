...
const LineManagerDashboardNew = ({ user }) => {
  ...
  const stats = {
    directReports: (data.directReports || []).length,
    pendingVerifications: data.pendingSkillVerifications || 0,
    approvalsNeeded: data.approvalsNeeded || 0,
    teamSize: (data.directReports || []).length,
  };

  const recentActivity = (data.activities || []).slice(0, 5);
  const teamMembers = (data.directReports || []).slice(0, 5);
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Your direct reports and pending approvals
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 divide-x divide-gray-100">
          <div className="px-6 py-5 hover:bg-rscm-dutch-white/30 transition-colors cursor-pointer group">
            <div className="flex items-baseline gap-2 mb-1">
              ...
              <UsersIcon size={14} className="text-rscm-violet opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          ...
          <div className="px-6 py-5 hover:bg-rscm-dutch-white/30 transition-colors cursor-pointer group">
            <div className="flex items-baseline gap-2 mb-1">
              ...
              {stats.pendingVerifications > 0 && (
                <AlertCircle size={14} className="text-rscm-plum opacity-0 group-hover:opacity-100 transition-opacity" />
              )}
            </div>
            <div className="text-xs text-gray-500">Skill verifications</div>
          </div>

          <div className="px-6 py-5 hover:bg-rscm-dutch-white/30 transition-colors cursor-pointer group">
            <div className="flex items-baseline gap-2 mb-1">
              <div className="text-2xl font-semibold text-rscm-dark-purple">
                {stats.approvalsNeeded}
              </div>
              {stats.approvalsNeeded > 0 && (
                <Clock size={14} className="text-rscm-plum opacity-0 group-hover:opacity-100 transition-opacity" />
              )}
            </div>
            <div className="text-xs text-gray-500">Approvals needed</div>
          </div>
          ...
          <div className="divide-y divide-gray-50">
            {teamMembers.length > 0 ? (
              ...
              <div className="px-6 py-12 text-center">
                <UsersIcon size={32} className="mx-auto mb-2 text-gray-400 opacity-50" />
...
export default LineManagerDashboardNew;
