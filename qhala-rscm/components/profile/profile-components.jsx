      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge className={getStatusColor(user?.availabilityStatus)} variant="outline">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        <div className="text-center">
          ...
          <div className="mt-1 h-2 bg-muted rounded-full overflow-hidden">
            <div
              className={cn("h-full transition-all duration-300", getCapacityColor(capacityData?.percentage || 0))}
              style={{ width: `${Math.min(capacityData?.percentage || 0, 100)}%` }}
            />
          </div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-foreground">{user?.weeklyHours || 40}h</div>
          <div className="text-sm text-muted-foreground">Weekly Hours</div>
          <div className="text-xs text-muted-foreground mt-1">{user?.employeeType || "Full-time"}</div>
        </div>
        <div className="text-center col-span-2 lg:col-span-1">
          <div className="text-2xl font-bold text-foreground">{leaveData?.remaining || 0}</div>
          <div className="text-sm text-muted-foreground">Leave Days</div>
          <div className="text-xs text-muted-foreground mt-1">{leaveData?.compDays || 0} comp days</div>
        </div>
      </div>
    </div>
  </motion.div>
...
export const EmploymentDetailsCard = ({ user, isEditing, onEdit, onSave, onCancel, isSaving }) => (
  <Card className="overflow-hidden">
        ...
        {!isEditing ? (
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={onSave} disabled={isSaving}>
              {isSaving ? <LoadingSpinner size={16} className="mr-2" /> : <Save className="h-4 w-4 mr-2" />}
              Save
            </Button>
          </div>
        )}
    ...
    <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="mt-1">
            {isEditing ? (
              ...
              <p className="text-foreground capitalize">{user?.employeeType || "Not specified"}</p>
            )}
          </div>
        </div>
        <div>
          ...
          <div className="mt-1">
            {isEditing ? (
              <Input type="number" defaultValue={user?.weeklyHours || 40} min="1" max="60" />
            ) : (
        ...
        <div>
          ...
          <div className="mt-1">
            {isEditing ? (
              <Input
                type="date"
                defaultValue={user?.contractStartDate ? new Date(user.contractStartDate).toISOString().split("T")[0] : ""}
              />
            ) : (
              <p className="text-foreground">
                {user?.contractStartDate ? new Date(user.contractStartDate).toLocaleDateString() : "Not specified"}
              </p>
            )}
          </div>
        </div>
        <div>
          ...
          <div className="mt-1">
            {isEditing ? (
              <Input
                type="date"
                defaultValue={user?.contractEndDate ? new Date(user.contractEndDate).toISOString().split("T")[0] : ""}
              />
            ) : (
              <p className="text-foreground">
                {user?.contractEndDate ? new Date(user.contractEndDate).toLocaleDateString() : "Ongoing"}
              </p>
            )}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="mt-1">
          {isEditing ? (
            <Textarea placeholder="Enter payment terms..." defaultValue={user?.paymentTerms || ""} rows={3} />
          ) : (
            <p className="text-foreground">{user?.paymentTerms || "Not specified"}</p>
          )}
        </div>
      </div>
    </CardContent>
    ...
    <CardContent className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="text-center p-3 rounded-lg bg-green-50 dark:bg-green-900/20">
          <div className="text-2xl font-bold text-green-700 dark:text-green-400">{leaveData?.remaining || 0}</div>
          <div className="text-sm text-green-600 dark:text-green-500">Annual Leave</div>
        </div>
        <div className="text-center p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20">
          <div className="text-2xl font-bold text-blue-700 dark:text-blue-400">{leaveData?.compDays || 0}</div>
          <div className="text-sm text-blue-600 dark:text-blue-500">Comp Days</div>
        </div>
      </div>
      <div className="space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium">{leaveData?.entitlement || 21} days</span>
        </div>
        <Progress value={((leaveData?.used || 0) / (leaveData?.entitlement || 21)) * 100} className="h-2" />
      </div>
    </CardContent>
  </Card>
);
        ...
        <div className="flex flex-wrap gap-2">
          {currentSkills?.length > 0 ? (
            currentSkills.map((skill) => (
              <Badge
                key={skill._id}
                className={cn("px-3 py-1.5 text-sm font-medium border", getSkillLevelColor(skill.proficiency))}
              >
                {skill.skillId?.name}
                {skill.proficiency && <span className="ml-1 opacity-75">L{skill.proficiency}</span>}
              </Badge>
            ))
          ) : (
            <p className="text-muted-foreground text-sm italic">No current skills added yet</p>
          )}
        </div>
      </div>
      <div>
        <h4 className="font-medium text-foreground mb-3 flex items-center gap-2">
          ...
          Learning Goals ({desiredSkills?.length || 0})
        </h4>
        <div className="flex flex-wrap gap-2">
          {desiredSkills?.length > 0 ? (
            desiredSkills.map((skill) => (
              <Badge key={skill._id} variant="outline" className="px-3 py-1.5 text-sm font-medium border-dashed">
                {skill.skillId?.name}
              </Badge>
            ))
          ) : (
            <p className="text-muted-foreground text-sm italic">No learning goals set yet</p>
          )}
        </div>
      </div>
    </CardContent>
          ...
          {allocations.allocations.map((allocation) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <Badge className={cn("text-xs", getAllocationPercentageColor(allocation.allocationPercentage))}>
              ...
              <div className="space-y-1 text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {allocation.startDate ? new Date(allocation.startDate).toLocaleDateString() : "Start date TBD"}
        ...
        <div className="space-y-3">
          {requests.map((request) => (
            <div key={request._id} className="p-3 rounded-lg border bg-card">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="font-medium capitalize">{request.requestType.replace("_", " ")} Request</p>
