// lib/notificationUtils.js - REPLACE the entire file
...
export const createNotification = async ({
  ...
  try {
    ...
    const link = customLink || generateNotificationLink(type, relatedResource, onModel);
...
export const createBulkNotifications = async (notifications) => {
  try {
    const notificationsWithLinks = notifications.map(notif => ({
      ...notif,
      link: notif.customLink || generateNotificationLink(
        notif.type, 
